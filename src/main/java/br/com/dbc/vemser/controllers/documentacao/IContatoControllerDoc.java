...
public interface IContatoControllerDoc {
    ...
    @Operation(summary = "Listar contatos por usuario logado", description = "Lista todos os contatos do usuario logado")
    @ApiResponses(
            value = {
                    @ApiResponse(responseCode = "200", description = "Contatos retornados com sucesso"),
                    @ApiResponse(responseCode = "403", description = "Acesso negado"),
    ...
    ResponseEntity<Page<ContatoResponseDTO>> listAllUsuario(@PageableDefault(page = 0, size = 10, sort = {"id"}) Pageable pageable,
                                                            @RequestParam(required = false) TipoEnum tipoEnum) throws BancoDeDadosException;
